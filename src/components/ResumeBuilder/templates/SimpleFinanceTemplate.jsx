  ...
  return (
    <div className="mt-2 flex gap-2">
      {[1, 2, 3, 4, 5].map((item) => (
        <span
          key={item}
          className={`h-[8px] flex-1 border border-[#d48806] ${
            item <= safeRating ? "bg-[#d48806]" : "bg-transparent"
          }`}
        />
      ))}
    </div>
...
const SectionTitle = ({ children }) => (
  <h2 className="mb-3 border-b border-[#d48806] pb-1 text-center text-[22px] font-bold text-[#d48806]">
    {children}
  </h2>
);

const SimpleFinanceTemplate = ({ resumeData }) => {
  const {
    personal = {},
    summary = "",
    experience = [],
      ...
      <header className="avoid-break text-center">
        {personal.photoUrl && (
          <img
            ...
            className="mx-auto h-44 w-44 rounded-xl border-[4px] border-[#e4c76f] object-cover"
          />
        )}
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-x-5 gap-y-2 text-[14px]">
          {personal.email && (
            <span className="flex items-center gap-1">
              <FiMail className="text-[#d48806]" /> {personal.email}
            </span>
          )}

          {personal.phone && (
            <span className="flex items-center gap-1">
              <FiPhone className="text-[#d48806]" /> {personal.phone}
            </span>
          )}

          {personal.location && (
            <span className="flex items-center gap-1">
              <FiMapPin className="text-[#d48806]" /> {personal.location}
            </span>
          )}

          {personal.linkedin && (
            <span className="flex items-center gap-1">
              <FiLinkedin className="text-[#d48806]" /> {personal.linkedin}
            </span>
          )}

          {personal.website && (
            <span className="flex items-center gap-1">
              <FiGlobe className="text-[#d48806]" /> {personal.website}
            </span>
          )}
        </div>
      </header>
...
export default SimpleFinanceTemplate;
