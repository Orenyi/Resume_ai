  ...
  return (
    <div className="mt-2 flex gap-2">
      {[1, 2, 3, 4, 5].map((dot) => (
        <span
          key={dot}
          className={`h-[10px] w-[10px] border border-[#4b5563] ${
            dot <= safeRating ? "bg-[#4b5563]" : "bg-transparent"
          }`}
        />
      ))}
    </div>
...
const SectionTitle = ({ children }) => (
  <h2 className="mb-3 text-[22px] font-bold text-[#4b5563]">{children}</h2>
);

const SimpleDesignTemplate = ({ resumeData }) => {
  const {
    personal = {},
    summary = "",
    experience = [],
  ...
  return (
    <section className="w-full min-h-[1123px] bg-white px-5 py-5 font-serif text-black">
      <header className="avoid-break border-b border-gray-700 pb-5">
...
export default SimpleDesignTemplate;
